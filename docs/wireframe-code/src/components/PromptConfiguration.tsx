...
Deliver a complete, professional resume in clean Markdown format ready for immediate use in job applications, optimized for ATS systems and flawless rendering across all platforms.`;
  ...
  return <Section title="Core Prompt Configuration" isOpen={true}>
      <div className="space-y-4">
        <div>
          <div className="flex justify-between items-center mb-2">
            <label className="font-medium text-zinc-600">System Prompt</label>
            <button onClick={handleReset} className="flex items-center gap-1.5 px-3 py-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-700 text-sm rounded-lg border border-zinc-300 transition-colors">
              <RotateCcw className="h-3.5 w-3.5" />
              Reset to Default
            </button>
          </div>
          <textarea className="w-full p-4 border border-zinc-300 rounded-lg h-40 focus:ring-2 focus:ring-red-700 focus:border-red-700 transition font-mono text-sm" placeholder="You are an AI resume builder..." value={promptValue} onChange={e => setPromptValue(e.target.value)} maxLength={10000} />
